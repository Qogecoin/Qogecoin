...
/** Call node.updateRwSetting() with Qogecoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const util::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Qogecoin 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/qogecoin/qogecoin/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because qogecoin 22.x will not try to read these.
...
//! Convert enabled/size values to qogecoin -prune setting.
...
//! Get pruning enabled value to show in GUI from qogecoin -prune setting.
...
//! Get pruning size value to show in GUI from qogecoin -prune setting. If
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if qogecoin.conf value
...
void OptionsModel::checkAndMigrate()
{
    ...
    // (https://github.com/qogecoin-core/gui/issues/567).
